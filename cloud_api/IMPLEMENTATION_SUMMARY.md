...
**Before:** yt-dlp executed with `execFile()` in main thread → blocks Node.js event loop → all HTTP
requests stall

**After:** Worker pool with 2-10 worker threads execute yt-dlp in parallel → main thread free to
handle HTTP
...
Successfully transformed a **single-threaded blocking API** into a **scalable, fault-tolerant
microservice** with:
