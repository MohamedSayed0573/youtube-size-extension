...
/**
 * Extracts YouTube cookies from the browser in Netscape format
 *
 * This is used to authenticate yt-dlp requests on the cloud server,
 * bypassing YouTube's bot detection which blocks datacenter IPs.
 * @async
 * @returns {Promise<string|null>} Cookies string in Netscape format, or null if unavailable
 */
async function extractYouTubeCookies() {
    try {
        ...
        Logger.info(`Extracted ${cookies.length} YouTube cookies`);
        return lines.join("\n");
    } catch (e) {
...
async function callCloudApi(url, durationHint) {
    ...
    try {
        ...
        // Extract and include YouTube cookies for yt-dlp authentication
        // This bypasses YouTube's bot detection on datacenter IPs
